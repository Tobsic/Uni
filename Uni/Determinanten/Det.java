public class Det{
	public static void main(String[] args) throws Exception{
		...
		Console.writeln();
		Console.writeln("Die Determinate ist: {0}", det(matrix));
	}
	...
	public static double det(double[][] matrix) throws Exception{
		...
		return detByGauﬂ(matrix);
	}
	private static double _det(double[][] matrix){
		int dimension = matrix.length;
		if(dimension == 2)
		...
		for(int selectedRow = 0; selectedRow < dimension; selectedRow++){
			...
			result *= (selectedRow % 2 * -2 + 1) * matrix[selectedRow][0] *  _det(subdeterminant);
		}
		return result;
	}
	public static double detByGauﬂ(double[][] matrix){
